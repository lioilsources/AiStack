...
| **spark-oom-guard** | hlavní pojistka: MemAvailable < 1 GiB 3 s, nebo (při MemAvailable < 2 GiB) PSI full ≥ 15 10 s / some ≥ 40 15 s → SIGKILL prvního živého cíle ze seznamu; když není koho zabít a tlak trvá 3 min → reboot (nejvýš 1× za hodinu) | `oom-guard/` |
