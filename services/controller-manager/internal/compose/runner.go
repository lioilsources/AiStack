...
type Runner interface {
	...
	Down(ctx context.Context, composeFile string, services []string) error
}
...
func (r *DockerRunner) Up(ctx context.Context, composeFile string, services []string) error {
	args := append([]string{"up", "-d", "--remove-orphans"}, services...)
	return run(ctx, composeFile, args...)
}
...
func (r *DockerRunner) Down(ctx context.Context, composeFile string, services []string) error {
	if len(services) == 0 {
		return run(ctx, composeFile, "down")
	}
	if err := run(ctx, composeFile, append([]string{"stop"}, services...)...); err != nil {
		return err
	}
	return run(ctx, composeFile, append([]string{"rm", "-f"}, services...)...)
}
